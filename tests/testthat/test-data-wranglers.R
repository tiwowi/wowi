...
testthat::test_that("returns the expected outputs when `.gam_based` is set to 'flag_wfhz' ", {
  ## Sample data ----
  x <- anthro |>
  ...
  testthat::expect_true(all(
    c("locationid", "latitude", "longitude") %in% names(r[[3]])
  ))
...
testthat::test_that("returns the expected outputs when `.gam_based` is set to 'flag_mfaz' ", {
  ## Sample data ----
  x <- anthro |>
    dplyr::rename(longitude = y, latitude = x) |>
    dplyr::filter(district == "Kotido") |>
  ...
  testthat::expect_true(sum(r[[2]][2]) == 260)
})

## ---- Case: `.gam-based = "combined"` ----------------------------------------

testthat::test_that("returns the expected outputs when `.gam_based` is set to 'combined' ", {
  ### Observed data ----
  x <- anthro |>
    dplyr::rename(longitude = y, latitude = x) |>
    dplyr::filter(district == "Kotido") |>
...
testthat::test_that("prepares and saves case, controls and geo files into a user-specified dir", {
  ### Sample data ----
  x <- anthro |>
    dplyr::rename(longitude = y, latitude = x) |>
    dplyr::filter(district == "Kotido") |>
  ...
  testthat::expect_message(
    ...
    regexp = paste0(
      "`",
      basename(out_dir),
      "` already exists in project repo."
    ),
    fixed = TRUE
  )
})
